...
buildah build --network=host -f "$VCS_PATH/taskcluster/docker/${NAME}/Dockerfile" -o type=tar,dest=/workspace/image.tar -t ${NAME}:final "$VCS_PATH/taskcluster/docker/${NAME}"
